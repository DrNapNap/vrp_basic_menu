	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(vehicleHandle) then
		  if GetVehicleDoorsLockedForPlayer(vehicleHandle,PlayerId()) or any then
			...
			vRP.notify({"~g~Køretøjet er låst op."})
			...
			vRP.notify({"~g~Køretøj allerede låst op."})
		  end
		else
			vRP.notify({"~r~Du er for langt væk fra køretøjet."})
		end
	end)
end
...
function vRPbm.deleteVehicleInFrontOrInside(offset)
  ...
  if IsEntityAVehicle(veh) then
    ...
    vRP.notify({"~g~Køretøjet blev fjernet."})
  else
